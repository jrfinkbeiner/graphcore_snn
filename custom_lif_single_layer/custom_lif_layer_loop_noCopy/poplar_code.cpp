...
void performBatchedLIFStateUpdate(poplar::Graph &graph, poplar::Tensor &weights, 
                            poplar::Tensor &state, poplar::Tensor &newState, BatchedSparseSpikes &inp_spikes, 
  ...
  for (unsigned irow = 0; irow < weights.dim(0); ++irow) {
    for (unsigned ibatch = 0; ibatch < batchsize; ++ibatch) {
      auto v = graph.addVertex(cs, poputil::templateVertex("LIFStateUpdate", dtype),
                                ...
                                {"new_state", newState[ibatch][irow]}});
...
void performLIFStepFworwardPass(poplar::Graph &graph, poplar::Tensor &weights, poplar::Tensor &prevState, poplar::Tensor &thisState, BatchedSparseSpikes &inp_spikes, 
                            poplar::Tensor &decay_constants, poplar::Tensor &thresholds, BatchedSparseSpikes &out_spikes,
                            poplar::program::Sequence &prog, const poplar::DebugNameAndId &dnai = {}) {
  
  performBatchedLIFStateUpdate(graph, weights, prevState, thisState, inp_spikes, decay_constants, thresholds, prog, dnai);
  // genBatchedLIFOutSpikesTopK(graph, new_state, thresholds, out_spikes, prog, dnai);
  genBatchedLIFOutSpikes2Threshs(graph, thisState, thresholds, out_spikes, prog, dnai);
...
extern "C" poplar::program::Program Build(
  ...
  // poplar::Tensor weights_inp = inputs[0];
  // poplar::Tensor init_state_inp = inputs[1];
  // poplar::Tensor inp_spike_ids_inp = inputs[2];
  // poplar::Tensor num_inp_spikes_inp = inputs[3];
  // poplar::Tensor decay_constatns_inp = inputs[4];
  // poplar::Tensor thresholds_inp = inputs[5];
  ...
  poplar::Tensor out_spike_ids = alloc_perneuron_3d(graph, {seq_len, batchsize, size_sparse_out}, dtype, 0, {dnai, "init_state"});
  poplar::Tensor num_out_spikes = alloc_perneuron_3d(graph, {seq_len, batchsize, 1}, dtype, 0, {dnai, "init_state"});
  poplar::Tensor stateSeqOutput = alloc_perneuron_3d(graph, {seq_len, batchsize, size_out}, dtype, 0, {dnai, "init_state"});
  ...
  // //----------------------------------------- REPEAT -------------------------------------------------  
  // auto loopFwd = [&graph, &weights, &decay_constatns, &thresholds, &init_state, &inp_spike_ids, &num_inp_spikes, &out_spike_ids, &num_out_spikes, &stateSeqOutput, &dnai] (
  //   unsigned itime,
  // ) {
  ...
  //   }
  //   poplar::Tensor thisState = stateSeqOutput[itime];
  //   BatchedSparseSpikes inpSpikes{inp_spike_ids[itime], num_inp_spikes[itime]};
  //   BatchedSparseSpikes outSpikes{out_spike_ids[itime], num_out_spikes[itime]};
  //   // poplar::program::Sequence loopIter = loopFwd(graph, inp_spikes, out_spikes, prevState, thisState, dnai);

  //   poplar::Tensor newState{performLIFStepFworwardPass(
  //       graph, weights, prevState, inpSpikes, decay_constatns, thresholds, outSpikes, loop, {dnai})};
  //   loop.add(poplar::program::Copy(newState, thisState, false, {dnai, "copy new state"}));
  //   return loop;
  // };

  // poplar::program::Sequence cloop = popops::countedLoop(graph, seq_len, loopFwd, dnai);
  // fwdProg.add(cloop);
...
poplar::program::Program Build_grad(
  ...
  //------------------------------------------- loop -------------------------------------------------  
  auto loopBwd = [&weights, &decay_constatns, &thresholds, &dLdweights](
                     poplar::Graph &graph,
                     BatchedSparseSpikes fwdInpSpikes,
                     BatchedSparseSpikes fwdOutSpikes,
                     poplar::Tensor &fwdState,
                     poplar::Tensor &dLdnextState,
                     poplar::Tensor &dLdfwdOutSpikes,
                     poplar::Tensor &dLdInpSpikes,
                     const poplar::DebugNameAndId &dnai) {
    
    auto loop = poplar::program::Sequence{{}, {dnai}};

    performLIFStepBackwardPass(
        graph, weights, fwdState, fwdInpSpikes, decay_constatns, thresholds, fwdOutSpikes, dLdweights, dLdnextState, dLdfwdOutSpikes, dLdInpSpikes, loop, {dnai});
