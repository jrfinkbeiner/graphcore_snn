    ...
    do
        ...
        do
            ...
            do
		     ...
		     # for DATASET_NAME in NMNIST SHD
		     # do
		          python3 benchmarking_script.py --use_ipu=1 --impl_method=sparse_layer --profile_run=0 --max_activity=$MAX_ACTIVITY --batchsize=$BATCHSIZE --lr=$LEARNING_RATE --transpose_weights=1 --second_thresh=$SECOND_THRESH --num_hidden_layers=$NUM_HIDDEN_LAYERS --bench_mode=multi_layer --weight_mul=2.0 --sparse_size_inp=32 --dataset_name=NMNIST
                  python3 benchmarking_script.py --use_ipu=1 --impl_method=sparse_ops --profile_run=0 --max_activity=$MAX_ACTIVITY --batchsize=$BATCHSIZE --lr=$LEARNING_RATE --transpose_weights=1 --second_thresh=$SECOND_THRESH --num_hidden_layers=$NUM_HIDDEN_LAYERS --bench_mode=multi_layer --weight_mul=2.0 --sparse_size_inp=32 --dataset_name=NMNIST
	     	     # done
                done 
            done
        done
