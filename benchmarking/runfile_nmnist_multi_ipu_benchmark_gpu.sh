...
do
    ...
    do
        for VARIABLE in 16
        do
            # for NUM_IPUS in 1 2 4
            for NUM_IPUS in 2 3 4 5
            do
                # CUDA_VISIBLE_DEVICES="3" python3 train_nmnist.py --use_ipu=0 --impl_method=dense --profile_run=0 --sparse_multiplier=$VARIABLE --batchsize=$BATCHSIZE --lr=$LEARNING_RATE --transpose_weights=1 --num_ipus=$NUM_IPUS
